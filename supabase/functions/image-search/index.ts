...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (source === 'unsplash') {
      ...
      if (!res.ok) return new Response(JSON.stringify({ error: `Unsplash error: ${res.status}` }), { status: 502, headers: corsHeaders })
    ...
    if (source === 'pixabay') {
      ...
      if (!res.ok) return new Response(JSON.stringify({ error: `Pixabay error: ${res.status}` }), { status: 502, headers: corsHeaders })
      const data = await res.json()
