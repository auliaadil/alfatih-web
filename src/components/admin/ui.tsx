...
export function compareRows(a: any, b: any, key: string, dir: SortDir): number {
    const get = (obj: any, k: string): any => k.split('.').reduce((o: any, part) => o?.[part], obj);
        ...
        <th style={width ? { width } : undefined} className={`px-6 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide text-${align} ${className}`}>
            {isSortable ? (
                <button
                    ...
                    className="w-full text-left uppercase tracking-wide hover:text-gray-700 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 rounded"
                >
